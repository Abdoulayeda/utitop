                ...
                <div class="bg-gray-800 dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-500 dark:bg-gray-700">
                                <tr>
                                    <th scope="col" class="px-4 py-3 text-white">Numéro</th>
                                    <th scope="col" class="px-4 py-3 text-white">Prénom & nom</th>
                                    <th scope="col" class="px-4 py-3 text-white">Entreprise</th>
                                    <th scope="col" class="px-4 py-3 text-white">Adresse</th>
                                    <th scope="col" class="px-4 py-3 text-white">Téléphone</th>
                                    <th scope="col" colspan="3" class="px-4 py-3 text-white">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                               ...
                               <tr class="border-b dark:border-gray-700 text-white">
                                    ...
                                    <th scope="row" class="px-4 py-3 font-medium whitespace-nowrap dark:text-white">{{ $client->prenom }} {{ $client->nom }}</th>
                                   
                                    <td class="px-4 py-3">{{$client->entreprise}} </td>
                                    <td class="px-4 py-3">{{$client->adresse}} </td>
                                    <td class="px-4 py-3">{{$client->telephone}} </td>
                                    <td class="px-4 py-3">
                                       {{--  <a href="{{ route('commerciale.client.show', $client->id) }}">
                                        ...
                                        <div class="flex justify-center m-5">
                                            <button id="readProductButton{{$client->id}}" data-modal-toggle="readProductModal{{$client->id}}" class="block text-white  hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" type="button">
                                        ...
                                        @include('modals.show-client')
                                    </td>
                                    <td class="px-4 py-3">
                                        <a href="{{ route('commerciale.client.edit', $client->id) }}">
                                    ...
                                    <td class="px-6 py-4 text-red-800">
                                        <form action="{{ route('commerciale.client.delete', $client->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                               <button type="submit" >
